...
object ZkUtils extends Logging {
  ...
  def setupCommonPaths(zkClient: ZkClient) {
    for(path <- Seq(ConsumersPath, BrokerIdsPath, BrokerTopicsPath, TopicConfigChangesPath,
      TopicConfigPath, DeleteTopicsPath, BrokerSequenceIdPath))
   ...
   * we add 1000 to the return value from getSequenceId for the backward compatability.
   * users can provide brokerId in the config , inorder to avoid conflicts between zk generated
   * seqId and config.brokerId we increment zk seqId by 1000. This is a arbitary number will
   * be going away in the future versions as we completely rely on zk seqId.
   */
  def getBrokerSequenceId(zkClient: ZkClient): Int = {
    getSequenceId(zkClient, BrokerSequenceIdPath) + 1000
  }
