...
object SSLAuthUtils extends Logging {
  private var context: SSLContext = null
  private val initialized = new AtomicBoolean(false)

  def sslContext: SSLContext = {
    if (!initialized.get)
      throw new IllegalStateException("SSL authentication is not initialized.")
    context
  }
  ...
  def initializeSSLAuth(config: SSLConnectionConfig) {
    if(!initialized.get) {
      ...
      info("SSL authentication initialization has been successfully completed")
    } else
